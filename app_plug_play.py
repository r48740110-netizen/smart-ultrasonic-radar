...
<aside class="sidebar">
  ...
  <nav class="nav">
    ...
    <button data-page="connect">🔌 CONNECT</button>
...
<section id="connect" class="page">
  <div class="hero"><div><h2>Connect Hardware</h2><p>Plug your compatible radar project into the platform and start monitoring.</p></div><div class="actions"><button class="btn primary" onclick="refreshSerialPorts()">↻ Detect Devices</button><button class="btn" onclick="autoConnectArduino()">⚡ Auto Connect</button></div></div>
  <div class="connect-grid">
    <div class="card connect-card">
      <span class="eyebrow">STEP 1 · CHOOSE SOURCE</span>
      <div class="connect-options">
        <button class="connect-option active" id="optUsb" onclick="selectConnection('usb')"><span style="font-size:24px">🔌</span><strong>USB Arduino</strong><small>Serial data from Arduino/compatible boards.</small></button>
        <button class="connect-option" id="optBrowser" onclick="selectConnection('browser')"><span style="font-size:24px">🌐</span><strong>Browser USB</strong><small>Direct USB connection from Chrome/Edge.</small></button>
        <button class="connect-option" id="optNetwork" onclick="selectConnection('network')"><span style="font-size:24px">📶</span><strong>ESP32 / Network</strong><small>Send sensor readings to the platform API.</small></button>
      </div>
      <div id="usbSetup" style="margin-top:20px">
        <div class="connect-step"><div class="step-no">1</div><div><b>Detect your device</b><div style="color:var(--muted);font-size:12px;margin-top:4px">Connect the hardware to the student's laptop and click Detect Devices.</div></div></div>
        <div class="connect-step"><div class="step-no">2</div><div><b>Select connection</b><div style="color:var(--muted);font-size:12px;margin-top:4px">Choose the detected COM port and baud rate in Settings.</div></div></div>
        <div class="connect-step"><div class="step-no">3</div><div><b>Start monitoring</b><div style="color:var(--muted);font-size:12px;margin-top:4px">The same sensor data feeds Radar, Safety, Alerts and Analytics.</div></div></div>
        <div class="protocol-box">Supported: 75,42<br>{"angle":75,"distance":42}<br>angle=75,distance=42</div>
        <div class="actions" style="margin-top:15px"><button class="btn primary" onclick="refreshSerialPorts()">↻ Detect Arduino</button><button class="btn" onclick="autoConnectArduino()">⚡ Auto Connect</button><button class="btn primary" onclick="connectBrowserUSB()">🌐 Browser USB</button></div>
      </div>
      <div id="networkSetup" style="display:none;margin-top:20px">
    ...
    <div class="card connect-card">
      ...
      <div class="settings-help" style="margin-top:12px"><b>Plug & Play rule:</b> compatible hardware only needs to provide angle + distance. Your application performs visualization, threat classification, alerts and analytics.</div>
    </div>
  </div>
</section>
...
function page(name){document.querySelectorAll('.page').forEach(p=>p.classList.toggle('active',p.id===name));document.querySelectorAll('.nav button').forEach(b=>b.classList.toggle('active',b.dataset.page===name));if(name==='analytics'){analyze();drawCharts()}if(name==='alerts'){renderAlerts()}if(name==='connect'){refreshSerialPorts();updateConnectStatus()} }
...
function selectConnection(kind){
  document.querySelectorAll('.connect-option').forEach(x=>x.classList.remove('active'));
  $('opt'+(kind==='usb'?'Usb':kind==='browser'?'Browser':'Network')).classList.add('active');
  $('usbSetup').style.display=kind==='network'?'none':'block'; $('networkSetup').style.display=kind==='network'?'block':'none';
}
function updateConnectStatus(){
  const live=sensorMode==='LIVE'; $('connectState').textContent=live?'LIVE SENSOR':'SIMULATION MODE'; $('connectSource').textContent=live?(browserSerialRunning?'BROWSER USB':'ARDUINO / SERIAL'):'DEMO / SIMULATION'; $('connectAngle').textContent=(current.angle||0).toFixed(0)+'°'; $('connectDistance').textContent=(current.distance||0).toFixed(1)+' cm'; $('connectThreat').textContent=current.threat||'LOW';
}
const _oldUpdateUI=updateUI; updateUI=function(d){_oldUpdateUI(d); updateConnectStatus();};
