...
DATA_FILE = os.path.join(BASE_DIR, 'radar_data.csv')
app = Flask(__name__)
...
def add_reading(angle, distance, source='demo'):
    u=current_user()
    if not u: return
    c=db(); c.execute('INSERT INTO readings(user_id,ts,angle,distance,source) VALUES(?,?,?,?,?)',(u['id'],time.time(),angle,distance,source)); c.commit(); c.close()

def get_readings(limit=5000):
    u=current_user()
    if not u: return []
    c=db(); rows=c.execute('SELECT ts,angle,distance,source FROM readings WHERE user_id=? ORDER BY id DESC LIMIT ?', (u['id'],limit)).fetchall(); c.close()
    return [dict(r) for r in reversed(rows)]

def demo_reading(step):
    # Four stable targets plus background, 0=left, 90=top, 180=right
    targets={30:55,75:90,120:45,155:70}
    angle=(step*5)%181
    d=random.uniform(180,340)
    for a,v in targets.items():
        if abs(angle-a)<=5: d=max(5,v+random.uniform(-3,3))
    return angle,round(d,2)

LOGIN='''<!doctype html><html><head><meta name="viewport" content="width=device-width,initial-scale=1"><title>Smart Ultrasonic Radar</title><style>body{margin:0;background:#05080b;color:#e9f2f5;font-family:Segoe UI,Arial;display:grid;place-items:center;min-height:100vh}.box{width:min(390px,90vw);padding:32px;background:#0b1117;border:1px solid #1c2b34;border-radius:16px}h1{color:#39ff88;font-size:21px;letter-spacing:2px;margin:0 0 7px;text-align:center}p{text-align:center;color:#7f919b;font-size:12px;margin:0 0 25px}label{display:block;font-size:12px;color:#aebbc3;margin:13px 0 6px}input{width:100%;box-sizing:border-box;padding:12px;background:#070c10;color:white;border:1px solid #263640;border-radius:8px}button{width:100%;margin-top:18px;padding:12px;border-radius:8px;border:1px solid #1caa68;background:#0b1d15;color:#39ff88;font-weight:700;cursor:pointer}.switch{text-align:center;margin-top:16px;font-size:12px}.switch a{color:#39ff88}.err{color:#ff6c7b;background:#250f14;border:1px solid #6b2934;padding:9px;border-radius:7px;margin-top:12px;font-size:12px;text-align:center}</style></head><body><div class="box"><h1>SMART ULTRASONIC RADAR</h1><p>MONITORING SYSTEM</p><form method="post"><label>Username</label><input name="username" required autofocus placeholder="Enter username"><label>Password</label><input type="password" name="password" required placeholder="Enter password"><button>LOGIN</button>{% if error %}<div class="err">{{error}}</div>{% endif %}</form><div class="switch">New student? <a href="{{url_for('signup')}}">Create account</a></div></div></body></html>'''
...
def login():
    if request.method=='POST':
        ...
        return render_template_string(LOGIN,error='Invalid username or password')
...
def signup():
    if request.method=='POST':
        username=request.form.get('username','').strip(); password=request.form.get('password','')
        if len(username)<3 or len(password)<4: return render_template_string(LOGIN,error='Username must be 3+ chars and password 4+ chars')
        try:
            c=db(); cur=c.execute('INSERT INTO users(username,password) VALUES(?,?)',(username,password)); c.commit(); uid=cur.lastrowid; c.close(); session['user_id']=uid; return redirect(url_for('home'))
        except sqlite3.IntegrityError:
            return render_template_string(LOGIN,error='Username already exists')
    return render_template_string(LOGIN,error='Create your account by entering a new username and password.')
...
(function(){'use strict';
const $=id=>document.getElementById(id); let running=false, demo=false, serialPort=null, reader=null, step=0, last=null, targets=new Map(); let timer=null;
...
function nav(name){document.querySelectorAll('.page').forEach(p=>p.classList.toggle('active',p.id===name));document.querySelectorAll('.nav button').forEach(b=>b.classList.toggle('active',b.dataset.page===name));location.hash=name;if(name==='analytics')stats();if(name==='alerts')renderAlerts();if(name==='radar')resize();}
...
function start(){if(running)return;running=true;demo=true;$('sys').textContent='DEMO RUNNING';$('demoStatus').textContent='Demo mode is running.';loop()}
function stop(){running=false;demo=false;if(timer)clearTimeout(timer);timer=null;$('sys').textContent='SYSTEM ONLINE';$('radarState').textContent='● STANDBY';$('status').textContent='READY';}
$('start').addEventListener('click',start);$('stop').addEventListener('click',stop);$('demo').addEventListener('click',()=>running?stop():start());
$('usb').addEventListener('click',async()=>{if(!('serial' in navigator)){ $('usbStatus').textContent='Web Serial needs Chrome or Edge.';return}try{serialPort=await navigator.serial.requestPort();await serialPort.open({baudRate:115200});$('usbStatus').textContent='Arduino connected.';running=true;demo=false;$('sys').textContent='ARDUINO CONNECTED';readSerial();}catch(e){$('usbStatus').textContent='Connection cancelled or failed.';}});
async function readSerial(){if(!serialPort||!serialPort.readable)return;reader=serialPort.readable.getReader();let buf='';try{while(serialPort&&running){const {value,done}=await reader.read();if(done)break;buf+=new TextDecoder().decode(value);let lines=buf.split(/\\r?\\n/);buf=lines.pop();for(const line of lines){let x=line.trim();if(!x)continue;try{let j=JSON.parse(x);process(j,'arduino')}catch(e){let p=x.split(',');if(p.length>=2)process({angle:p[0],distance:p[1]},'arduino')}}}}catch(e){$('usbStatus').textContent='Arduino connection lost.'}finally{try{reader.releaseLock()}catch(e){}}}
async function stats(){const r=await fetch('/api/stats');const d=await r.json();$('total').textContent=d.total;$('detected').textContent=d.detected;$('rate').textContent=d.rate+'%';$('avg').textContent=d.average+' cm';$('aTotal').textContent=d.total;$('aDetected').textContent=d.detected;$('aRate').textContent=d.rate+'%';$('aAvg').textContent=d.average+' cm';$('mix').textContent='Danger '+d.high+' · Warning '+d.warning+' · Safe '+d.safe}
async function renderAlerts(){const r=await fetch('/api/alerts');const rows=await r.json();$('alertRows').innerHTML=rows.map(x=>'<tr><td>'+new Date(x.ts*1000).toLocaleTimeString()+'</td><td>'+Math.round(x.angle)+'°</td><td>'+Math.round(x.distance)+' cm</td><td style="font-weight:700;color:'+(x.level==='HIGH'?'#ff4057':x.level==='MEDIUM'?'#ffc857':'#39ff88')+'">'+x.level+'</td></tr>').join('')||'<tr><td colspan="4">No readings yet.</td></tr>'}
...
save(); if(location.hash)nav(location.hash.slice(1)); else nav('home'); resize(); draw('radar'); draw('radar2'); setInterval(()=>{if(running)stats()},1200); stats();
})();
</script></body></html>'''
...
@login_required
def stats_api():
    rows=get_readings(); ds=[r['distance'] for r in rows]
    high=sum(d<60 for d in ds); warn=sum(60<=d<120 for d in ds); safe=sum(d>=120 for d in ds); det=high+warn
...
@login_required
def alerts():
    rows=get_readings(50); out=[]
    for r in reversed(rows):
        d=r['distance']; out.append({'ts':r['ts'],'angle':r['angle'],'distance':d,'level':'HIGH' if d<60 else 'MEDIUM' if d<120 else 'LOW'})
    return jsonify(out)

@app.route('/api/health')
def health(): return jsonify(ok=True,service='Smart Ultrasonic Radar',version='3.0')
