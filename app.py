    ...
    if request.method=='POST':
        ...
        try:
            ...
            if USE_POSTGRES:
                ...
                uid=cur.lastrowid
            c.commit(); c.close(); session.clear(); session['user_id']=uid; return redirect(url_for('home'))
...
function start(){if(running)return;resetTarget();running=true;demo=true;$('sys').textContent='DEMO RUNNING';$('demoStatus').textContent='Demo mode is running.';loop()}
async function stop(){running=false;demo=false;if(timer)clearTimeout(timer);timer=null;if(reader){try{await reader.cancel()}catch(e){}try{reader.releaseLock()}catch(e){}reader=null}if(serialPort){try{await serialPort.close()}catch(e){}serialPort=null}$('sys').textContent='SYSTEM ONLINE';$('radarState').textContent='● STANDBY';$('status').textContent='READY';$('usbStatus').textContent='Arduino not connected.';resetTarget();}
$('start').addEventListener('click',start);$('stop').addEventListener('click',stop);$('demo').addEventListener('click',()=>running?stop():start());
$('usb').addEventListener('click',async()=>{if(!('serial' in navigator)){ $('usbStatus').textContent='Web Serial needs Chrome or Edge over HTTPS or localhost.';return}if(running)await stop();try{serialPort=await navigator.serial.requestPort();await serialPort.open({baudRate:115200});$('usbStatus').textContent='Arduino connected — receiving data.';running=true;demo=false;$('sys').textContent='ARDUINO CONNECTED';$('radarState').textContent='● LIVE';readSerial();}catch(e){serialPort=null;$('usbStatus').textContent='Connection cancelled or failed.';}});
